...
(function ( $ ) {
  ...
  $.fn.selectMe = function( options ) {
	...
	SelectMe.prototype._initDropdown = function() {
		...
		self._initSearch();
		self._initAlphabets();	//edited by Prashanth --added filter by letter.
		self._initShortcuts();
		self._initOptions();
	...
	/*
	 * Render Letters to filter by letter --added function by Prashanth
	 */
	SelectMe.prototype._initAlphabets = function() {
		var self = this;
		if (self.options.search) {
			var $alphabetWrapper = $('<div class="selectme-alphabets" style="text-align:center;"></div>').appendTo(self.$dropdown);

			var alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
			var alphabet_array = alphabet.split("");
			var str = '';
			for(var i = 0; i< alphabet_array.length; i++){
				str += '<a class="alpha-btns" href="#" ><span class="badge">' + alphabet_array[i] + '</span></a>&nbsp';
			}
			self.$alphabet = $(str).appendTo($alphabetWrapper);

			$('.alpha-btns>.badge').on('click', function(){			
				//self.$search[0].value = this.textContent;
				var selected_letter = this.textContent;
				//trigger change event programatically
				//self.$search.onChange();
				// var evt = document.createEvent("HTMLEvents");
				// evt.initEvent("keyup", true, true);
				// self.$search.dispatchEvent(evt); // for DOM-compliant browsers
				...
				self.$options.find('li:not(.optgroup)').each(function(){
					...
					// show option if string exists
					if( optText.toLowerCase().indexOf( selected_letter.toLowerCase() ) == 0 ) {
						$(this).show();
					  var $optGroup = $(this).closest('.optgroup');
						if ($optGroup) {
						  $optGroup.show();
		...
		for( var key in options ) {
			...
			if( thisOption.hasOwnProperty('value') ) { // option
				...
				$thisCheckbox
					...
					//added to close the dropdown when an option is click-selected.
					.click(function(){
						self.$select.find('option[value="' + $(this).val() + '"]').prop('selected', $(this).prop('checked'));
						self._updatePlaceholderText();
						self.options.onOptionValueChanged(this);
